...
const fetchCart = createAsyncThunk(
    "fetchCart",async(user_id,{rejectWithValue})=>{
        const {data,error} = await supabase.from("cart").select("*,products(image_url,price,title,discount,stock,profiles(profile_photo,display_name,id))").eq("user_id",user_id)
